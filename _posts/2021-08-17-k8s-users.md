...
```
တကယ်လို့ ကျွန်တော်တို့ကသာ admin.conf file ကို ~/.kube အောက်မထည့်ခဲ့ဖူးဆိုရင် kubectl get nodes --kubeconfig=/etc/kubernetes/admin.conf ဆိုပြီး kubeconfig ရဲ့ path လမ်းကြောင်းကိုထည့်ပေးရမှာဖြစ်ပါတယ်။ admin.conf က အဲ့ဒီ cluster ရဲ့ information တွေစုထားတဲ့ config file ဖြစ်ပြီး ~/.kube/config ကတော့ kubernetes admin တစ်ယောက်ကနေ cluster တွေကို centralized control လုပ်ဖို့ထည့်ထားတဲ့ config file ဖြစ်ပါတယ်။ cluster တစ်ခုထဲကိုသုံးနေရင်တော့ သိပ်ပြီးမသိသာပါဘူး။ ဆိုလိုတာက အဲ့ဒီ ~/.kube/config ထဲမှာ တစ်ခြားသော cluster တွေရဲ့ information တွေလည်းပါနိုင်ပါတယ်။ ဒါဆိုရင် အဲ့ဒီ ~/.kube/config ကို cat နဲ့ တစ်ချက်လောက်ကြည့်လိုက်ရအောင်။ 
...
139682675926336:error:0909006C:PEM routines:get_name:no start line:../crypto/pem/pem_lib.c:745:Expecting: ANY PRIVATE KEY
```
<h2> Create Context For Demouser </h2>

ပြီးသွားရင် demouser အတွက် context တစ်ခုကို create ပေးပါမယ်။ အရင်ဆုံး ~/.kube/config ထဲကို demouser အရင် create ပေးရပါမယ်။ demouser ကို access ပေးသုံးမယ့် cluster က config ထဲမှာရှိပြီးသားဖြစ်လို့ cluster ကို config ထဲထည့်ပေးဖို့မလိုပါဘူး။ user ကို ထည့်ဖို့အတွက် အောက်ကအတိုင်းလုပ်ပေးရပါမယ်။
...
```
အားလုံးပြီးသွားရင်တော့ demouser အတွက် context ကို create လို့ရပါပြီ။ context ကို demo-context လို့နာမည်ပေးလိုက်ပါမယ်။ 
...
```
ဒါဆိုရင်အခု ကျွန်တော့်ဆီမှာ context (၂)ခုရှိနေပါပြီ။ cluster က kubernetes ဆိုတဲ့ တစ်ခုတည်းပါ။ cluster ကို access မယ့် user က (၂)ယောက် ။ admin user နဲ့ demouser။ ဒါဆို contexts တွေကို ကြည့်လိုက်ရအောင်။
...
```
demouser အတွက် cluster မှာလုပ်ပိုင်ခွင့်ရှိတဲ့ role တွေကိုသတ်မှတ်ပေးရပါမယ်။ admin context ကိုပြန်ပြီး switch လုပ်ပေးရပါမယ်။ admin user ကသာ role တွေကို create ပေးလို့ရပါတယ်။
...
- apiGroups: [""] # "" indicates the core API group
  ...
  verbs: ["get", "watch", "create", "delete", "update", "list"]
...
```  
rolebiding creat ပြီးသွားရင်တော့ demo namespace မှာ nginx deployment ကို create ကြည့်ပါ။ 
